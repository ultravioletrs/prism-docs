...
Prism is a comprehensive multi-party computation platform that provides a computation management service for confidential computing using Trusted Execution Environments (TEEs). It integrates with [CocosAI](https://www.ultraviolet.rs/docs/cocos-ai/) to enable secure, private computations across multiple parties while maintaining data confidentiality and computation integrity.
