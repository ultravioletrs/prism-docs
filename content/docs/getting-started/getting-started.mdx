...
Use the [Cocos CLI tool](https://www.ultraviolet.rs/docs/cocos-ai/cli#command-keys):
...
Each role has specific actions that determine what users can do—like `view`, `edit`, `run`, `algo_provider`, `dataset_provider`, or `result_consumer`. [Learn more about managing roles and permissions](/computations/roles/).
...
> **📝 Note**: Users must already be invited to the workspace before they can be assigned computation roles. A user cannot belong to more than one role, to add permissions you should actions on the required [role](/computations/roles/).
...
For each file, generate its cryptographic hash using the [Cocos CLI](https://www.ultraviolet.rs/docs/cocos-ai/cli#command-checksum) to be used in the next step:
...
More documentation on: [algo](https://www.ultraviolet.rs/docs/cocos-ai/cli/#command-algo) and [data](https://www.ultraviolet.rs/docs/cocos-ai/cli/#command-data)
...
Once the computation completes, download and decrypt your results using the [Cocos CLI tool](https://www.ultraviolet.rs/docs/cocos-ai/cli/#command-result) with your **private key** as a digital identity:
...
1. **Explore Advanced Features**: Learn about [computation roles](/computations/roles/), advanced security settings
2. **API Integration**: Discover how to automate workflows using our [API](/api-reference/api/)
3. **Attestation**: Discover our [attestation](https://www.ultraviolet.rs/docs/cocos-ai/attestation-introduction) implementation
4. **Community**: Connect with other Prism and [CoCoS](https://www.ultraviolet.rs/docs/cocos-ai/) users

Ready to dive deeper? Check out our [Computation documentation](/computations/computations/) and [API Documentation](/api-reference/api/).
