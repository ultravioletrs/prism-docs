...
// Next.js's image pipeline -- see source.config.ts, where fumadocs-mdx's
// remark-image plugin is disabled because it statically imports every
// markdown image at build time, which requires the file to exist on disk.
// These images live in R2 instead (see worker.ts, scripts/README.md).
// Rendered as a plain, zoomable <img> -- no next/image, no width/height
// needed, so there's nothing to keep in sync when images change.
